describe('Well status', () => {
    ...
    const pipSetPoint = 120;
    const allowedStroke = 3;
    const startUpStroke = 1;
    const secondarypipSetPoint = 90;

    it('Check the work of PIP Mode', () => {
        // test commands
        function commands() {
        ...
        function checkSecondary() {
            onOffModbusUnit(0);
            cy.get('.mat-select').eq(0).click();
            cy.get('.mat-option-text').contains("PIP").click();
            cy.wait(3000);
            cy.get('[formgroupname="vfdUse"] .mat-slide-toggle-input')
                ...
                .then((e) => {
                    if (!e[0].checked) {
                        ...
                        cy.get('.mat-select').eq(2).click().type('02').type('{enter}');
                        cy.get('.mat-slide-toggle').eq(0).click();
                        cy.get('.mat-input-element').eq(3).clear().type(secondarypipSetPoint);
                        cy.get('#saveControl').click();
                        cy.wait(3000);
                        cy.reload();
                        cy.wait(4000);
