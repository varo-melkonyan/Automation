...
describe('Custom Malfunction', () => {
    ...
    it('Check the work of Custom Malfunction', () => {
        ...
        async function setCustomMalf() {
            ...
            await checkChannels(1);
        }

        async function checkChannels(custom) {
            customMalf = custom;
            await cy.get('.mat-checkbox-input').eq(0).click({force: true, multiple: true});
            if (custom === 1) {
                await cy.get('.mat-input-element').eq(0).clear().type(30000);
            }
        ...
        async function checkMalfunction(retries, retriesTimeH, retriesTimeMin, currentRetries, malfItem) {
            await cy.get(".footer-status__value").eq(4).then(async (e) => {
                if (!e[0].innerText.includes("Violation")) {
                    ...
                    await cy.wait(2000);
                    await cy.get('.mat-checkbox-input').click({force: true, multiple: true});
                    await cy.get('.mat-input-element').eq(0).clear().type(23276);
                    await cy.get('#saveMalf').click();
                    await cy.wait(3000);
                    await cy.get('.malfunction-button').click({force: true});
                    await cy.wait(2000);
                    await cy.get('.malf-delete').eq(0).click();
                    await cy.wait(2000);
                    await cy.get('.malf-delete').eq(1).click();
                    await cy.wait(2000);
                    await cy.get('.malf-delete').eq(0).click();
                }
                else {
